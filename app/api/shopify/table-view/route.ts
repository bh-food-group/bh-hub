...
export async function GET(request: NextRequest) {
  try {
    ...
    const query = parseOfficeTableListQuery(sp, 'shopify');
    const { rows, total } = await queryShopifyTableRows(query, offset, limit);
    return NextResponse.json({ ok: true, rows, total });
  } catch (err: unknown) {
    return toApiErrorResponse(err, 'GET /api/shopify/table-view error:');
  }
}
